...
myAverages %>%
  ...
  geom_smooth(aes(y = avg_min_per_day), method = "lm", fill = NA, color = "darkred", size = 1.5) +
...
myTC_files <- list()
myAverages_TC <- data.frame(year = numeric(),
...
for (y in 5:21) {
 ...
 myTC_files[[y-2]] <- mySummary_files[[y]]  %>%
   ...
   #travel cost incurred per min reported traveling for recreation (nuemrator of mean)
 ...
 #get standard errors 
 myAverages_TC <- bind_rows(myAverages_TC, get_SE(myTC_files[[y-2]], y))
...
myAverages_TC_calcs <- left_join(myAverages_TC, myPop, by = "year") %>%
...
ggplot(myAverages_TC_calcs, aes(x=year, y=annual_TC_value)) + 
    geom_errorbar(aes(ymin=annual_TC_value-annual_TC_CI, 
                      ymax=annual_TC_value+annual_TC_CI), width=.1, position=pd) +
