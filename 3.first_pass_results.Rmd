...
myMinutes %>%
  ...
  geom_smooth(aes(y = avg_min_per_day), method = "lm", fill = NA, color = "black", size = 1.5) +
...
myTable_1 <- myTC %>%
  ...
  select(!race) %>%
  rename(`Daily Value` = avg_travel_cost_0) %>%
  rename(`Std. Err.` = standard_error) %>%
  rename(`Annual Value (Bill. $)` = annual_TC_value_bill) %>%
  rename(`Std. Err` = annual_TC_SE_bill)

myTable_1 %>%
  stargazer(title = "Table 1: National Estimates of Daily Recreation Value and Annual Value",
            summary = FALSE, 
            digit.separator= "", 
