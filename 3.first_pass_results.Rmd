...
for (y in 3:21) {
  myAverages$avg_min_per_day[y-2] <- get_avg_rec_min(mySummary_files[[y]])
...
my2021 <- my2021_og %>%
  ...
  filter(!is.na(indv_travel_cost)) %>% # a lot of people don't report their income 
  mutate(travel_cost_weight_0 = indv_travel_cost*sample_weight) 
...
sum(my2021$travel_cost_lower_bound_weighted, na.rm = TRUE) #20 million, but 50% are missing 


# -----------------------------------------------------------------------------
# get ready to do standard erros. 
# -----------------------------------------------------------------------------


myWeights <- vroom("raw_data/ATUS_2003-2021_Replicate_weights/21.csv") 

myTest_21 <- left_join(my2021, myWeights, by = c("household_id" = "TUCASEID"))

myTest_21 <- myTest_21 
for (i in 1:length(myTest_21)) {
  tc_num <- as.numeric(myTest_21[i,9])
  myTest_21[i,11:170] <- myTest_21[i,11:170]*tc_num
}

# get_standard_error() is written but the loop is super duper slow. May need to figure out better way or run on plan and store dataframes! 

#next step, the actual calculation (probably need to pivot longer... that may be the better way to speed things up as well!!) Next steps, rethink with the pivot longer 
...
- multiply by population to get the lower bound estimate 
...
# -----------------------------------------------------------------------------
# Have already read all summary file into mySummary_files
# Functions are in files 
# myAverages data frame already exists 
...
for (y in 3:21) {
 myTC_files[[y-2]] <- mySummary_files[[y]]  %>%
    select(household_id, race, week_earning, sample_weight, total_min_rec, total_min_travel) %>%
    mutate(week_earning = ifelse(week_earning < 0 , NA, week_earning)) %>%
    mutate(OC_time_per_hour = round(week_earning / 40 * (1/3), 2)) %>% # opportunity cost of time
    mutate(OC_time_per_min = round(OC_time_per_hour/60, 4)) %>%
...
myAverages_TC <- data.frame(year = myYear)

for (y in 3:21) {
  
  #national average
  myAverages_TC$avg_travel_cost[y-2] <- get_avg_travel_cost(myTC_files[[y-2]])
  ...
  #average broken down by race 
  myRaceList <- list()
  for (r in 1:5) {
    myDF_temp <-myTC_files[[y-2]] %>%
      filter(race == r)
    
    if (r == 1){
      myAverages_TC$avg_travel_cost_white[y-2] <- get_avg_travel_cost(myDF_temp)
    } else if (r == 2){
      myAverages_TC$avg_travel_cost_black[y-2] <- get_avg_travel_cost(myDF_temp)
    } else if (r == 3){
      myAverages_TC$avg_travel_cost_native[y-2] <- get_avg_travel_cost(myDF_temp)
    } else if (r == 4){
      myAverages_TC$avg_travel_cost_asian[y-2] <- get_avg_travel_cost(myDF_temp)
    } else if (r == 5){
      myAverages_TC$avg_travel_cost_island[y-2] <- get_avg_travel_cost(myDF_temp)
    }
   
     rm(myDF_temp) # should be getting replaced, but just to make sure
  }
}
