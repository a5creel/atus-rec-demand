...
myTable_1 <- myTC %>%
  ...
  select(!race) %>%
  rename(`Daily Value ($ 1)` = avg_travel_cost_0) %>%
  rename(`Std. Err.` = standard_error) %>%
  rename(`Ann. Value ($ Bill)` = annual_TC_value_bill) %>%
...
myTable_summary <- left_join(myMin_table, myValues, by = c("Race" = "race")) %>%
    rename(`Daily Value ($ 1)` = avg_value_daily) %>%
    rename(`Ann. Value ($ Bill)` = avg_value_annual) %>%
