...
myMinutes_long <- myMinutes %>%
  rename(Population = avg_min_per_day) %>%
  ...
  pivot_longer(cols = c(Population, White, Black,Native, Islander, Asian), values_to = "avg_min") %>%
...
ggplot(myMinutes_long, aes(x = year, y = avg_min, colour = Race)) +
  ...
  ggtitle("Time recreating varries by race", "Black Americans recreate outdoors less") +
  ylab("Minutes") +
  xlab("Year")+
  labs(caption = "Data Source: ATUS Microdata Activity Summary files, '03-'21")+
  theme_bw() + 
  ...
  coord_cartesian(ylim = c(0, 30))+  #zoom
    scale_color_manual(values=c('darkcyan', 'darkgoldenrod4', 'darkmagenta', 'darkolivegreen4', 'black', 'darkred'))
...
#Combine race with average 

myTable <- bind_rows(myTC, myTC_race) %>%
  select(race, year, avg_travel_cost_0, standard_error, annual_TC_value, annual_TC_SE) %>%
  mutate(avg_travel_cost_0 = round(avg_travel_cost_0, 2))%>%
  mutate(standard_error = round(standard_error, 3))%>% 
  mutate(annual_TC_value_bill = round(annual_TC_value / 1e9,2)) %>%
